...
VerifyService straight_verify_service(const Runtime& rt, CertificateProvider& cert_provider, CertificateValidator& certs, Backend& backend, CertificateCache& cert_cache, SignHeaderPolicy& sign_policy, PositionProvider& positioning)
{
    return [&](VerifyRequest&& request) -> VerifyConfirm {
        ...
        if (PayloadType::Signed != secured_message.payload.type) {
            ...
            std::cerr << "[VERIFY] FAIL Unsigned_Message (payload type not Signed)\n";
            return confirm;
        }

        if (2 != secured_message.protocol_version()) {
            confirm.report = VerificationReport::Incompatible_Protocol;
            std::cerr << "[VERIFY] FAIL Incompatible_Protocol (version=" << secured_message.protocol_version() << ")\n";
            return confirm;
        }
        ...
        if (!its_aid) {
            ...
            confirm.report = VerificationReport::Incompatible_Protocol;
            std::cerr << "[VERIFY] FAIL Incompatible_Protocol (no ITS-AID header field)\n";
            return confirm;
        }
            ...
            switch (get_type(*signer_info)) {
                ...
                case SignerInfoType::Certificate_Chain:
                {
                    ...
                    if (chain.size() == 0) {
                        ...
                        std::cerr << "[VERIFY] FAIL Signer_Certificate_Not_Found (empty Certificate_Chain)\n";
                        return confirm;
                        ...
                        std::cerr << "[VERIFY] FAIL Invalid_Certificate (Certificate_Chain too long: " << chain.size() << ")\n";
                        return confirm;
                    }
                    ...
                    for (auto& cert : chain) {
                        ...
                        if (cert.subject_info.subject_type == SubjectType::Authorization_Authority) {
                            ...
                            if (!check_certificate_time(cert, rt.now()) || !check_certificate_region(cert, positioning.position_fix())) {
                                confirm.report = VerificationReport::Invalid_Certificate;
                                std::cerr << "[VERIFY] FAIL Invalid_Certificate (AA in chain: time or region check failed)\n";
                                return confirm;
                            }
        ...
        if (possible_certificates.size() == 0) {
            ...
            std::cerr << "[VERIFY] FAIL Signer_Certificate_Not_Found (no possible certs, signer digest not in cache), aid=" << confirm.its_aid << "\n";
            return confirm;
        }

        const Time64* gen_time_field = secured_message.header_field<HeaderFieldType::Generation_Time>();
        if (!check_generation_time(secured_message, rt.now())) {
            confirm.report = VerificationReport::Invalid_Timestamp;
            std::cerr << "[VERIFY] FAIL Invalid_Timestamp aid=" << confirm.its_aid
                      << " gen_time=" << (gen_time_field ? *gen_time_field : 0)
                      << " now=" << convert_time64(rt.now()) << "\n";
            return confirm;
        }
        ...
        if (!signature) {
            confirm.report = VerificationReport::Unsigned_Message;
            std::cerr << "[VERIFY] FAIL Unsigned_Message (no Signature trailer field)\n";
            return confirm;
        }

        if (PublicKeyAlgorithm::ECDSA_NISTP256_With_SHA256 != get_type(*signature)) {
            confirm.report = VerificationReport::False_Signature;
            std::cerr << "[VERIFY] FAIL False_Signature (wrong PublicKeyAlgorithm)\n";
            return confirm;
        }
        ...
        if (!ecdsa || ecdsa->s.size() != field_len) {
            confirm.report = VerificationReport::False_Signature;
            std::cerr << "[VERIFY] FAIL False_Signature (bad ecdsa size: ecdsa=" << (bool)ecdsa
                      << " s.size=" << (ecdsa ? ecdsa->s.size() : 0) << " expected=" << field_len << ")\n";
            return confirm;
        }
        ...
        for (const auto& cert : possible_certificates) {
            ...
            if (subject_type != SubjectType::Authorization_Ticket) {
                ...
                std::cerr << "[VERIFY] FAIL Invalid_Certificate (cert subject_type=" << static_cast<int>(subject_type) << " is not AT)\n";
                return confirm;
            }
            ...
            if (!public_key) {
                ...
                std::cerr << "[VERIFY] FAIL Invalid_Certificate (Missing_Public_Key in AT cert)\n";
                return confirm;
            }

            bool msg_sig_ok = backend.verify_data(public_key.get(), payload, *ecdsa);
            std::cerr << "[VERIFY] backend.verify_data (msg sig) => " << msg_sig_ok << " payload_len=" << payload.size() << "\n";
            if (msg_sig_ok) {
                signer = cert;
                break;
            }
        ...
        if (!signer) {
            ...
            if (signer_info && get_type(*signer_info) == SignerInfoType::Certificate_Digest_With_SHA256) {
                ...
                confirm.report = VerificationReport::Signer_Certificate_Not_Found;
                std::cerr << "[VERIFY] FAIL Signer_Certificate_Not_Found (verify_data failed, digest signer), aid=" << confirm.its_aid << "\n";
            } else {
                // signature does not match the certificate received with this message
                confirm.report = VerificationReport::False_Signature;
                std::cerr << "[VERIFY] FAIL False_Signature (verify_data failed, full cert signer), aid=" << confirm.its_aid << "\n";
            }
        ...
        if (!check_generation_location(secured_message, *signer)) {
            ...
            std::cerr << "[VERIFY] FAIL Invalid_Certificate Off_Region (generation location)\n";
            return confirm;
        }
        ...
        if (!check_certificate_time(*signer, rt.now())) {
            ...
            std::cerr << "[VERIFY] FAIL Invalid_Certificate Off_Time_Period (AT cert time check)\n";
            return confirm;
        }
        ...
        if (!check_certificate_region(*signer, positioning.position_fix())) {
            confirm.report = VerificationReport::Invalid_Certificate;
            confirm.certificate_validity = CertificateInvalidReason::Off_Region;
            std::cerr << "[VERIFY] FAIL Invalid_Certificate Off_Region (AT cert region check)\n";
            return confirm;
        }

...
bool check_subject_assurance_consistency(const Certificate& certificate, const Certificate& signer)
{
    ...
    // Assurance_Level is optional. Only enforce ordering when both certs carry it.
...
bool check_consistency(const Certificate& certificate, const Certificate& signer)
{
    if (!check_time_consistency(certificate, signer)) {
        std::cerr << "[CERT] check_consistency FAIL: time (cert start/end must be within signer start/end)\n";
        return false;
    }

    if (!check_permission_consistency(certificate, signer)) {
        auto cert_aids = extract_application_identifiers(certificate);
        auto sign_aids = extract_application_identifiers(signer);
        std::cerr << "[CERT] check_consistency FAIL: permissions (cert AIDs not subset of signer AIDs)\n";
        std::cerr << "[CERT]   cert AIDs:";
        for (auto a : cert_aids) std::cerr << " " << a;
        std::cerr << "\n[CERT]   signer AIDs:";
        for (auto a : sign_aids) std::cerr << " " << a;
        std::cerr << "\n";
        return false;
    }

    if (!check_subject_assurance_consistency(certificate, signer)) {
        std::cerr << "[CERT] check_consistency FAIL: subject assurance\n";
        return false;
    }

    if (!check_region_consistency(certificate, signer)) {
        std::cerr << "[CERT] check_consistency FAIL: region\n";
        return false;
    }
...
CertificateValidity DefaultCertificateValidator::check_certificate(const Certificate& certificate)
{
    ...
    // Assurance_Level is optional; certify-generated certs omit it so we do not
    // hard-reject here. Consistency with the signer is checked in check_consistency.
